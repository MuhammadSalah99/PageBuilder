import { Editor, RichTextEditorAction } from "grapesjs";
import Piklor from "./colorPicker";
import { RTE } from "./interface";
import './Styles/RTE.css'

export default (editor: Editor, opts = {}) => {
  const options: Partial<RTE> = {
    ...{
      // default options
      base: {
        bold: true,
  ...
  if (options.maxWidth) {
    const toolBar = rte.getToolbarEl() as HTMLElement;
    if (toolBar && toolBar.firstChild instanceof HTMLElement) {
      toolBar.firstChild!.style.maxWidth = options.maxWidth as string;
    }
  }
  ...
  options.fonts &&
    ...
    rte.add("fontName", {
      ...
      result: (rte, action: RichTextEditorAction) =>
        rte.exec("fontName", action.btn?.firstChild?.value),
      // Callback on any input change (mousedown, keydown, etc..)
      update: (rte, action: RichTextEditorAction) => {
        const value = rte.doc.queryCommandValue(action.name); // what is the value here?
        if (value != "false") {
          // value is a string
  ...
  options.fonts &&
    ...
    rte.add("fontColor", {
      ...
      result: (rte) => {
        ...
        pk1.colorChosen((col: string) => rte.exec("foreColor", col));
      },
    });
  let pk2: Piklor | null = null;
  options.fonts &&
    ...
    rte.add("hiliteColor", {
      ...
      result: (rte) => {
        ...
        pk2.colorChosen((col: string) => rte.exec("hiliteColor", col));
      },
    });
  options.format &&
